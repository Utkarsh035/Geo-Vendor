<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    <head>
        ...
        <title>Vendor Registration | GeoVendor</title>
        ...
        <%@include file="/common_html/all_css.html" %>
            ...
            <script>
                ...
                function checkDigit(event) {
                    ...
                    if (data.length >= 10) { Swal.fire({ title: "Limit Reached", text: "Only 10 digits allowed", icon: "warning" }); event.preventDefault(); return; }
                    if (!/^[0-9]*$/.test(data + event.key)) { Swal.fire({ title: "Format Error", text: "Only Digits are allowed", icon: "error" }); event.preventDefault(); }
                }
            </script>
    </head>

    <body>
        <%@include file="/WEB-INF/common/nav_bar.html" %>
            <div class="reg-wrapper">
                <div class="reg-card">
                    <h2 class="text-center">Vendor Registration</h2>
                    <form action="/GeoVendor/Vendor_Registration" method="post" novalidate class="needs-validation"
                        ...
                        <div class="text-center mb-3">
                            <button class="btn-platinum-submit w-100"><i class="fas fa-user-plus"></i> Register</button>
                        </div>
                    </form>
                </div>
            </div>
...
%>
                        ...
                        setTimeout(() => { toast.classList.remove('show'); setTimeout(() => { toast.remove(); }, 700); }, 5000);
<% } %>
                </script>
